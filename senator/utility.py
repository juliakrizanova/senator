...
def get_utility(previous_utility: np.ndarray, chosen_resources: list[int], 
    ...
    if owner_trashold <= 1.0:
        is_owner = previous_utility >= owner_trashold * votes #original variant of the model, where is_owner is computed by using a trashold
       
    current_utility = ((previous_utility * owner_loss) * is_owner) + (previous_utility * other_loss) * (1- is_owner)
    ...
    for i in range(RESOURCES):
        for j in range(RESOURCES):
            for k in range(RESOURCES):
                utility_matrix[:, i, j, k] = get_utility(previous_utility, [i,j,k], votes, is_owner, 
                                                          owner_loss, other_loss, owner_trashold).sum(axis=1)
