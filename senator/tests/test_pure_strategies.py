...
def test_pure_strategy():
    ...
    game_fixed = Game(initial_utility, votes, is_owner, 0.9, 0.5, 2.0)
    ...
    assert np.all(np.isin(strategies_fixed, [0,1])), "The fixed-ownership model should have pure strategies"
    assert np.all(np.isin(strategies_trashold, [0,1])), "The trashold-ownership model should have pure strategies"

def test_pure_strategy_real_game():
    ...
    for _ in range(iterations): 
        ...
        assert np.all(np.isin(strategies, [0,1])), f"The senator game does not have pure strategies for: own:{owner_loss}, oth:{other_loss}, trh:{owner_trashold} strategies: {strategies}"