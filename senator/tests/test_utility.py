...
def test_get_utility_shape():
    #Checking that the utility has the correct shape
    assert get_utility(np.zeros((3,17)), [1,2,3], np.zeros((17)), np.zeros((17))).shape == (3,17)

def test_get_utility_computation():
...
def test_get_utility_owner_loss():
    ...
    assert get_utility(previous_utility, [1,1,1], np.ones(17), np.zeros((3,17)), 0.5, 0, 1)[0][0] == 0.5
    ...
    assert get_utility(previous_utility, [1,1,1], np.ones(17), is_owner, 1, 0, 2)[0][0] == 1

def test_get_utility_other_loss():
    ...
    assert get_utility(previous_utility, [1,1,1], np.ones(17), np.zeros((3,17)), 0, 1, 1)[0][0] == 0.5

    #Checking the utility is correctly computed based on the other_loss in fixed-ownership model
    previous_utility = np.zeros((3,17))
    previous_utility[0] = np.ones(17)
    assert get_utility(previous_utility, [1,1,1], np.ones(17), np.zeros((3,17)), 0, 1, 2)[0][0] == 1

def test_get_utility_matrix_shape():
    #Checking that the utility matrix has the correct shape
    assert get_utility_matrix(np.zeros((3,17)), np.zeros((17)), np.zeros((3,17))).shape == (3,17,17,17)

def test_get_utility_matrix():
...
def test_utility_overflow():
    ...
    one_day_utility = get_utility(previous_utility, chosen_resources, votes, is_owner, 0.8, 0.5, 2.0)
    assert (one_day_utility.sum(axis=0) > votes).sum() == 0, f"Utility overflow detected."
    #Test in model with trashold-ownership
    one_day_utility = get_utility(previous_utility, chosen_resources, votes, is_owner, 0.8, 0.5, 0.35)
    assert (one_day_utility.sum(axis=0) > votes).sum() == 0, f"Utility overflow detected."